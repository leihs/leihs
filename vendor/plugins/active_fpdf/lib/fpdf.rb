...
class FPDF
    FPDF_VERSION = '1.53d'
    FPDF_FONTPATH = 'app/views/fonts'
    FPDF_IMAGES_PATH = 'public/images/'
    ...
    def AddFont(family, style='', file='')
        ...
        if FPDF.const_defined? 'FPDF_FONTPATH'
    ...
    def Image(filename,x,y,w,h=0,type='',link='')
        ...
        unless @images.has_key?(filename)
            ...
            if type==''
                ...
                type=filename[pos+1..-1]
            end
            type.downcase!
            if type=='jpg' or type=='jpeg'
                info=parsejpg(FPDF_IMAGES_PATH + filename)
            elsif type=='png'
                info=parsepng(FPDF_IMAGES_PATH + filename)
            else
    ...
    def putfonts
        ...
        @FontFiles.each do |file, info|
            ...
            if FPDF.const_defined? 'FPDF_FONTPATH' then
