...
# Schritte des Reserviervorgangs
	...
	def ausleihe_pruefen
		...
		unless @user.ausweis and @user.ausweis.length > 0
			...
			@reservation_parameter = params[ :reservation ]
			@reservation_parameter[ :zubehoer ] = nil if params[ :reservation ][ :zubehoer ] and params[ :reservation ][ :zubehoer ].length < 1
			# wenn Reservation noch neu ist, auf genehmigt setzen,
			# denn sie wird herausgeben
			@reservation_parameter[ :status ] = 2 if @reservation.status == 0
			
			unless @reservation.update_attributes( @reservation_parameter )
				flash[ :alarm ] = 'Reservation konnte nicht geändert werden'
				redirect_to :action => 'herausgeben', :id => @reservation.id
			else
				Logeintrag.neuer_eintrag( session[ :user ], 'prüft Ausleihe',
	...
	def ausleihe_eintragen
		reservation_vorbereiten
			...
			if @reservation.save
				...
				render :action => 'ausleihe_pruefen'
			end
	end
...
# Schritte des Rücknahmevorgangs
	...
	def ruecknahme_pruefen
		...
		logger.debug( "I --- ausleihen_con | ruecknahme_pruefen -- params:#{params.to_yaml}" )
		logger.debug( "I --- ausleihen_con | ruecknahme_pruefen -- @reservation1:#{@reservation.to_yaml}" )
		if params[ :paket ]
		...
		if params[ :zubehoer ] and params[ :zubehoer ][ :zurueck ] == '0'
			@reservation_offen ||= @reservation.duplikat_ohne_pakete_zubehoer
			@reservation_offen.zubehoer = @reservation.zubehoer
			@reservation.zubehoer = nil
		end
		
		logger.debug( "I --- ausleihen_con | ruecknahme_pruefen -- @reservation2:#{@reservation.to_yaml}" )
		...
		logger.debug( "I--- reservation3: #{@reservation.to_yaml}")
		if @reservation.save
