...
class Notification < ActiveRecord::Base
  ...
  def self.order_received(order, send_mail = false)
    Mailer::Order.received(order).deliver_now if (send_mail and Setting.deliver_order_notifications)
  end
  ...
  def self.deadline_soon_reminder(user, reservations, send_mail = true)
    reservations.map(&:inventory_pool).uniq.each do |inventory_pool|
      o = Mailer::User.deadline_soon_reminder(user, inventory_pool, reservations).deliver_now if send_mail
