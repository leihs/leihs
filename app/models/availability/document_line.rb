...
#################################
    ...
    def available?
      ...
      if end_date < Date.today # check if it was never handed over
        ...
        # the order_lines should exclude each other
        reservations = model.order_lines.scoped_by_inventory_pool_id(inventory_pool).unsubmitted.running(start_date).by_user(order.user)
        reservations.delete(self)
        other_quantities = reservations.collect(&:quantity).sum
        (maximum_available_quantity - other_quantities) >= quantity
      else
