...
describe Authenticator::HsluAuthenticationController do
  ...
  context "if the user does not yet exist" do
    it "should be able to create a normal with various useful data grabbed from LDAP" do
      ...
      User.find(:first, :conditions => { :login => "normal_user" }).should_not == nil
    end
  end
  ...
  context "when dealing with users for the Video group" do
    it "should assign users to the group if they have the right displayName" do
      post :login, {:login => { :user => "video_user", :password => "1234" }}, {}
      user = User.find(:first, :conditions => { :login => "video_user" })
      user.should_not == nil
      user.groups.include?(Group.find(:first, :conditions => {:name => "Video"})).should == true
    end
    it "should not assign users to the group if they don't have the right displayName" do
      post :login, {:login => { :user => "normal_user", :password => "1234" }}, {}
      user = User.find(:first, :conditions => { :login => "normal_user" })
      user.should_not == nil
      user.groups.include?(Group.find(:first, :conditions => {:name => "Video"})).should == false
    end
  end
  ...
  context "when the user has a unique_id that is completely numeric" do
    it "should append an 'L' to the start of the user's unique_id and use that instead" do 
      post :login, {:login => { :user => "numeric_unique_id_user", :password => "1234" }}, {}
      user = User.find(:first, :conditions => { :login => "numeric_unique_id_user" })
      user.should_not == nil
