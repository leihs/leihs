class CreateRunningLinesView < ActiveRecord::Migration
  def up
    ...
    execute("CREATE VIEW running_lines AS " \
              "SELECT order_lines.id, 'OrderLine' AS type, orders.inventory_pool_id, model_id, quantity, start_date, end_date, " \
                "FALSE AS is_late, " \
              ...
              "WHERE (orders.status_const = #{Order::SUBMITTED}) AND (end_date >= CURDATE()) " \
