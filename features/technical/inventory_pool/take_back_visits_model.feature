Feature: Take back visits
  ...
  Scenario: Inventory pool should return a list of take back visits per user
    ...
    When all the contract lines of all the visits are combined
    Then one should get the set of contract lines that are associated with the users' contracts

  Scenario: Inventory pool should return a visit containing contract lines for items reserved from the same day on by a user
    ...
    When the take back visits of the given inventory pool are fetched
    Then the first 2 contract lines should be grouped inside the 1st visit, which makes it two visits in total

  Scenario: Inventory pool should not mix visits of different users
    Given there are 2 different contracts with lines for 2 different users
    ...
    When the take back visits of the given inventory pool are fetched
    Then the first 2 contract lines should now be grouped inside the 1st visit, which makes it 2 visits in total
