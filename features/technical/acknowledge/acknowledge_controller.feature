Feature: Acknowledge controller
  ...
  Add a line to a contract during acknowledge process

  Background:
    Given personas existing
    And test data setup for acknowledge controller test

  Scenario: Adds a line to the contract by providing a inventory code
    Given I log in as 'pius' with password 'password'
    When one adds a line to a contract by providing a inventory code
