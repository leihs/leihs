...
Scenario: Nur diejenigen Gegenstände und Optionen beim Hinzufügen automatisch anhaken, deren Startdatum heute ist.
...
Scenario: Barcode scanner: Piepsen, wenn etwas hinzugefügt wurde. Böse piepsen, wenn etwas nicht hinzugefügt werden konnte.
...
Scenario: Neues Frontend: Mitteilung bei "Submit Order" anzeigen (nur für ZHdK-User): "Mit dem abschicken dieser Bestellung akzeptieren Sie die "Richtlinie zur Ausleihe von Sachen der ZHdK" sowie jegliche abteilungsspezifische Ausleih-Richtlinien. 
...
Scenario: Eigenschaften: Sets von Eigenschaften sollte man unter einem Namen abspeichern können.
  Given pending - reported by Claudio on 22.10.2009

Scenario: Erinnerungsmails auch an alternative E-Mail-Adressen schicken. Funktioniert nur bei ZHdK-Benutzern, da können wir die alternative Mailadresse aus Evento holen (email_alt)
  Given pending - reported by Claudio on 4.1.2010

Scenario: Inventarmanager Level 3 sollen das Modell eines bestehenden Gegenstands ändern können. 
  Given pending - reported by Mike on 27.8.2009, confirmed by Florian/HKB on 9.8.2010

Scenario: Mehr Spalten sollen sortierbar sein. Welche genau, das weiss Claudio.
  Given pending - reported by Claudio before the dawn of time

Scenario: Maximale Ausleihzeit pro Modell. Benutzer sollen diese Modelle niemals länger buchen können. Manager sollen gewarnt werden, wenn sie längere Zeiten angeben. 
  Given pending - reported by Claudio an aeon ago

Scenario: Wenn ein Paket zurückkommt, Paket-Einzelteile überprüfen können, so dass man unvollständige Pakete entdecken kann.
  Given pending - reported by Claudio (?) long ago

Scenario: Eine bestehende Bestellung sollte man duplizieren können, um die gleichen Modelle in der gleichen Anzahl nochmals bestellen zu können.
  Given pending - reported by Claudio long ago

Scenario: Backend: Bestellungen optisch hervorheben, deren Enddatum gleich ist wie das Anfangsdatum einer anderen Bestellung.
  Given pending - reported by Claudio when dinosaurs still reigned

Scenario: Warnung anzeigen, wenn unvollständige Gegenstände ausgehändigt werden sollen. 
