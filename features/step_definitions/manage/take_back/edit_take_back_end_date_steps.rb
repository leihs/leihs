...
When /^I open a take back which has multiple lines$/ do
  @customer = @current_inventory_pool.users.find {|x| x.reservations_bundles.signed.exists? and !x.reservations_bundles.signed.detect{|c| c.lines.size > 1 and c.inventory_pool == @current_inventory_pool}.nil? }
  @contract = @customer.reservations_bundles.signed.detect{|c| c.lines.size > 1 and c.inventory_pool == @current_inventory_pool}
