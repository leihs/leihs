...
Then(/^the model list is unfiltered$/) do
  ...
  within '#model-list' do
    expect(all('.text-align-left').map(&:text).reject{|t| t.empty?}).to eq @current_user.models.from_category_and_all_its_descendants(@category.id).default_order.map(&:name)
  end
end
