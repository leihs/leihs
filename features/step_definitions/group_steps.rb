...
# Models in Groups
Then "that Model should not be $state in any other Group"  do |state|
...
Then /^(\w+) item(s?) of that Model should be (\w*) in Group '([^"]*)'( only)?$/ do |n, plural, state, group_name, exclusivity|
  ...
  quantities = AvailabilityChange.maximum_in_state_in_period( @model,
                                                              ...
                                                              (DateTime.now + 10.years),
                                                              state )
  quantities[group_name].should == n
...
When /^I move (\w+) item(s?) of that Model from Group "([^"]*)" to Group "([^"]*)"$/ do |n, plural, from_group_name, to_group_name|
  ...
  n.times do
    AvailabilityChange.move(@model, from_group, to_group)
  end
