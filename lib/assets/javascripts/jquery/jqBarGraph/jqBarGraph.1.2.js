...
(function($) {
 			...
 			if(valueData instanceof Array){
				...
				if(arr.type=="multi"){
 				...
 				for (i in valueData){
 					...
 					fs = 0; // font size
 					valueElement = '';
 					if (arr.showValues && valueData[i] != 0 && sum(valueData) != 1){
 						sv = arr.prefix+valueData[i]+arr.postfix;
 						fs = 12; // font-size is 0 if showValues = false
 						valueElement = "<span class='text'>"+sv+"</span>";
 					}
 					o = "<div class='subBars"+el.id+"' style='height:"+heig+"px; background-color: "+arr.colors[i]+"; left:"+wid*i+"px; color:"+arr.showValuesColor+"; font-size:"+fs+"px' >"+valueElement+"</div>";
    ...
    var grid = "<div class='grid' style='position:absolute; height:"+options.height+"px; width:"+options.width+"px;'>";
        ...
        for(var i = 0; i < gridlines.length; i++){
          ...
          grid += "<span class='text' style='position: absolute; left: 0; top:-5px;'>"+gridlines[i]+"</span>";
          grid += "<hr/>";
          grid += "</div>";
        }
