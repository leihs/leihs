...
$(document).ready(function(){
  ...
  $(window).bind("resize scroll", function() {
    ...
    Dialog.followViewPortDelayTimer = setTimeout(function() {
      var _top = ( ( $(window).height()/2 ) - ( $(".ui-dialog ").height() ) ) + window.pageYOffset;
      var _left = ( ( $(window).width()/2 ) - ( $(".ui-dialog ").width()/2 ) );
        ...
        $(_dialog).bind("dialogopen", function(event, ui) {
            ...
            $(this).parent().offset({left: ( $(this).data("startLeft") - ( $(this).parent().width()/2 )), top: ( $(this).data("startTop") - $(this).parent().height()/2)});
             ...
             if ($("#main").height() < $(window).height()/2) {
                ...
                var _top = ( ( $("#main").height()/2 ) - ( $(this).parent().height() ) + $("#main").offset().top ) + window.pageYOffset;
                var _left = ( ( $("#main").width()/2 ) - ( $(this).parent().width()/2 ) + $("#main").offset().left );
            } else {
                var _top = ( ( $(window).height()/2 ) - ( $(this).parent().height() ) ) + window.pageYOffset;
