 ...
 * @param show_months: true                      // Disable/Enable showing months
 *
 * 
 * ***** EXAMPLE ****
 * 
 * $('#container').historicalSearch(firstdate, lastdate, { params });  
 * 
 * ******************
 * 
**/
 (function($){
  ...
  var hs_elements = {};
	...
	$.fn.historicalSearch = function(firstdate, lastdate, opts){
      ...
      if(hs_options[element.id].selected_year) {
        ...
        if(hs_options[element.id].show_months) {
        ...
        // nothing is selected
        for (date = firstdate.getFullYear(); date <= lastdate.getFullYear(); date++) {
          var _item = $("<a href='#'>"+date+"</a>");
              _item.data("year", date);
          $(container).append(_item);
        }
      }
