 ...
 * @param max: 10                   // The maximal amount of visible items until collapsing
  ...
  var sM_containers = {};
    ...
    setup_items = function(element) {
      ...
      $(element).children().each(function(index, item){
        if($(element).children().length > (sM_options[element.id].max) && index > (sM_options[element.id].min-1)) {
          $(hidden).append(item);
        } else {
          $(element).append(item);
        }
    ...
    show_more = function(element, toggle) {
      ...
      $("html,body").animate({ scrollTop: $(element).offset().top + sM_options[element.id].offset.top }, 1400, "swing");
