...
function BookingCalendar() {
  ...
  this.setupDayCells = function() {
    ...
    $('#fullcalendar').find(".fc-widget-content").bind("click", function(event) {
      ...
      // select immediately as end date if start date is disabled
      ...
      $(this).qtip({
        content: {
           text: $.tmpl("tmpl/dialog/calendar/target_selection",{}), // PROVIDE EMPTY HASH AS DATA OTHERWISE ERROR
        },
        position: {
           my: 'bottom center',
           at: 'top center',
  ...
  this.setupHistoryDates = function() {
    // move start date to today if its in the past
    if(BookingCalendar.start_date < moment().sod().toDate() && $("#start_date").is(":not(:disabled)")) {
      BookingCalendar.start_date = moment().sod().toDate();
      $("#start_date").val(moment(BookingCalendar.start_date).format(BookingCalendar.local.dateFormat));
    }
    ...
    if(BookingCalendar.end_date < BookingCalendar.start_date) {
      ...
      $("#end_date").val(moment(BookingCalendar.end_date).format(i18n.date.L));
    }
  }
  ...
  this.setupStartDate = function() {
    ...
    $("#start_date").bind("keyup change", function(event) {
      ...
      if(BookingCalendar.validateDate(value)) {
        ...
        var fullCalendar_date = moment($('#fullcalendar').fullCalendar("getDate"));
        ...
        // prevent start_date for getting set to an history value
        ...
        if(BookingCalendar.validateDate($(this).val()) && event.type == "keyup") {
          // check if the full calendar view shows the same month and year that is set
    ...
    $("#start_date").bind("focus", function() {
      ...
      if(BookingCalendar.validateDate(value)) {
        var date = moment(value, BookingCalendar.local.dateFormat);
        var current_date = moment().sod();
        var fullCalendar_date = $('#fullcalendar').fullCalendar("getDate");
      }
    });
  }
  ...
  this.setupEndDate = function() {
    ...
    $("#end_date").bind("keyup change", function(event) {
      ...
      if(BookingCalendar.validateDate(value)) {
        ...
        if(BookingCalendar.validateDate($(this).val()) && event.type == "keyup") {
          // check if the full calendar view shows the same month and year that is set
          if(date.month() != fullCalendar_date.getMonth() || date.year() != fullCalendar_date.getFullYear()) {
            BookingCalendar.gotoDate(date);
          }
        }
  ...
  this.setupDates = function() {
    ...
    BookingCalendar.start_date = moment($("#start_date").val(), BookingCalendar.local.dateFormat).toDate();
    BookingCalendar.end_date = moment($("#end_date").val(), BookingCalendar.local.dateFormat).toDate();
  }
  ...
  this.setupLines = function() {
    var line_ids = $('#fullcalendar').data("line_ids");
    if(line_ids) {
      var form = $($('#fullcalendar')).closest("form");
  ...
  this.setupView = function() {
    var fullCalendar_date = moment($('#fullcalendar').fullCalendar("getDate"));
    if($("#start_date").is(":disabled")) {
      // check if the full calendar view shows the same month and year that is set in the end_date
      var date = moment($("#end_date").val(), BookingCalendar.local.dateFormat);
      if(date.month() != fullCalendar_date.month() || date.year() != fullCalendar_date.year()) {
        BookingCalendar.gotoDate(date.toDate());
      }
    } else {
      // check if the full calendar view shows the same month and year that is set in the start_date
  ...
  this.setupPartitionSelector = function() {
    if($("select#partition option").length == 0) return true;
  ...
  this.setSelectedPartitions = function(val) {
    if(val.length == 0) {
      $('#fullcalendar').removeData("selected_partition_ids");
    } else {
      $('#fullcalendar').data("selected_partition_ids", JSON.parse(val));
    }
  ...
  this.viewDisplayFunction = function(view) {
    ...
    $("#fullcalendar .fc-content .fc-widget-content").each(function(index, element){
      ...
      if (availability_dates != undefined) {
      ...
      if (availability_dates != undefined) {
        ...
        if($('#fullcalendar').data("selected_partition_ids") != undefined) {
          available_quantity = 0;
          for (var i in most_recent_av[2]) { // each availability entry
            if($('#fullcalendar').data("selected_partition_ids").indexOf(most_recent_av[2][i].group_id) > -1 || most_recent_av[2][i].group_id == null || most_recent_av[2][i].group_id == 0) { // null or 0 is the group "general" (the everyone partition)
      ...
      if (availability_dates != undefined) {
        ...
        var class_names = ["available", "unavailable"];
      }
      ...
      if(moment(date).sod() >= moment().sod() && !BookingCalendar.isClosedDay(date) && !isHoliday) {
        ...
        if($($('#fullcalendar')).hasClass("multiple")){
          $(element).removeClass(class_names[1]).addClass(class_names[0]);
        ...
        } else if(availability_dates == undefined) {
          $(element).find('div.fc-day-content > div').text("✓");
        } else { // 
          ...
          if($('#fullcalendar').data("selected_partition_ids") != undefined) $(element).find('div.fc-day-content').append("<span class='total_quantity'>/"+total_quantity+"</span>");
        }
      } else {
  ...
  this.setAvDates = function(render) {
    var q = parseInt($("input#quantity").val());
  	if($("select#inventory_pool_id option:selected").data("availability_dates") == "undefined") {
	   var av = undefined;
  	} else {
